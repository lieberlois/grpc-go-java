...
func (server *LaptopServer) CreateLaptop(
	...
	if len(laptop.Id) > 0 {
		...
		if err != nil {
			return nil, status.Errorf(codes.Internal, "cannot generate new laptop uuid: %v", err)
		}
		laptop.Id = id.String()
	}

	// heavy processing for timeout test
	...
	// save laptop to in-memory store
...
func (server *LaptopServer) UploadImage(stream pb.LaptopService_UploadImageServer) error {
	...
	log.Printf("image %s with size %d was saved", imageID, imageSize)
...
func (server *LaptopServer) RateLaptop(stream pb.LaptopService_RateLaptopServer) error {
	for {
		...
		log.Printf("received a rate-laptop request: laptop id %s and score %.2f", laptopID, score)
		...
		if err != nil {
			return logError(status.Errorf(codes.Internal, "couldn't add rating: %v", err))
		}
...
func contextError(ctx context.Context) error {
	...
	case context.Canceled:
		return logError(status.Error(codes.Canceled, "request was canceled"))
